...
<div class="container">
    ...
    <div>
        @foreach ($timesheets->unique('uid') as $timesheet)
        <a href="{{ route('timesheet.show', $timesheet->uid) }}" title="show">{{ ($timesheet->uid) }}
    ...
    <table class="table table-bordered table-responsive-lg" id="timesheetlist">
        ...
        @foreach ($indtimesheets as $indtimesheet)
        {{-- {{ dd($indtimesheets) }} --}}
            <tr>
                <td>{{ ($indtimesheet->id) }}</td>
                <td>{{ ($indtimesheet->staffname) }}</td>
                <td>@nl2br($indtimesheet->stafftask)</td>
                <td>{{ ($indtimesheet->taskdate) }}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
